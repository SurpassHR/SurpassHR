[![SurpassHR's github streak](https://github-readme-streak-stats.herokuapp.com/?user=SurpassHR&theme=github_dark_dimmed)](https://github.com/SurpassHR/SurpassHR)

![Anurag's GitHub stats](https://github-readme-stats.vercel.app/api?username=SurpassHR&show_icons=true&icon_color=199861&show=reviews,discussions_started,discussions_answered,prs_merged,prs_merged_percentage&theme=github_dark_dimmed)

[![SurpassHR's GitHub trophy](https://github-profile-trophy.vercel.app/?username=SurpassHR&theme=onedark&&column=9&no-frame=true)](https://github.com/SurpassHR/SurpassHR)

### 👋 Hi, I'm SurpassHR
A C++ embedded developer and open-source enthusiast.

I specialize in embedded systems development with C++, have knowledge in Python, and am currently learning Rust.

![C++](https://img.shields.io/badge/-C%2B%2B-black?logo=C%2B%2B) ![Python](https://img.shields.io/badge/-Python-black?logo=python) ![Rust](https://img.shields.io/badge/-Rust-black?logo=rust)

### 🤩 Visitor count

[![SurpassHR's visitor count](https://count.getloli.com/get/@SurpassHR?theme=3d-num)](https://github.com/SurpassHR/Youtube_SignatureCipher_Decryptor)

### ✨ Skills

I am proficient in various programming languages and technologies, and always eager to explore new ones.

#### Core Skills
[![Core skills](https://skillicons.dev/icons?i=cpp,py,rust,linux,git)](https://skillicons.dev)

#### Tools & Platforms
[![Tools & Platforms](https://skillicons.dev/icons?i=docker,mysql,sqlite,ubuntu,visualstudio,vscode,git)](https://skillicons.dev)

#### Machine Learning & Digital Media
[![ML & Digital Media skills](https://skillicons.dev/icons?i=pytorch,opencv,blender,pr,ps,ai,au)](https://skillicons.dev)

### 🚀 Currently Working On / Seeking Collaborations

- Learning: Rust (actively)
- Seeking Collaborations: Open-source projects, embedded systems development, performance optimization

### 📞 Contact

I look forward to connecting with you! If you have any project collaborations, technical discussions, or career opportunities, please feel free to reach out via:

- **Gmail**: hu.rui0530@gmail.com